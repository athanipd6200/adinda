  ...
  <v-container>
        ...
        <v-container>
          ...
          >
            <form @submit.prevent="submit">
              ...
              <v-img contain style="max-width:80vw;" :src="url_base + '/api/gambar_pembuka_artikel/'+news_edit.gambar_pembuka_artikel" alt=""/>
    ...
    <v-dialog v-model="dialogNewsView" fullscreen hide-overlay style="z-index:2051" transition="dialog-bottom-transition">
      <v-card>
        ...
        <v-container fluid>
            ...
            <p><b>Tampilankan di Web</b> : {{ news.status_tampilan_artikel === 1 ? 'Ditampilkan' : 'Tidak ditampilkan' }}</p>
            <p><b>Verifikasi</b> : {{ news.status_verifikasi_artikel === 1 ? 'Terverifikas' : 'Belum diverifikasi' }}</p>
            <p><b>Tanggal Artikel</b> : {{ news.tanggal_artikel }}</p>
            <p><b>Teks Pengantar</b> : {{ news.teks_pembuka_artikel }}</p>
            <p><b>Penulis Artikel</b> : {{ news.penulis_artikel }} ({{ news.created_by }})</p>
    ...
    <v-dialog v-model="dialogNewsCreate" fullscreen hide-overlay style="z-index:2052" transition="dialog-bottom-transition">
      <v-card>
        ...
        <v-container>
          <validation-observer
            ref="observer_create_news"
            v-slot="{ invalid }"
          >
            <form @submit.prevent="submit">
              <h3 class="mt-2"><b>Keanggotaan dari Artikel</b></h3>
              <validation-provider
                v-slot="{ errors }"
                name="Keanggotaan"
                rules="required"
              ...
              <validation-provider
                ...
                rules="required"
              >
              <v-date-picker v-model="news.tanggal_artikel"></v-date-picker>
              </validation-provider>
              ...
              <v-img :src="imagePreview" style="max-width:80vw;" contain/>
    ...
    <v-data-table
      ...
      :items="desserts"
      :search="search_artikel"
      item-key="id_artikel"
      class="elevation-1"
    >
      <template v-slot:top>
        <h2 class="mx-4">DAFTAR ARTIKEL
          <v-btn
            class="mx-2 my-2"
            fab
            dark
  ...
  export default {
    ...
    methods: {
      ...
      async editItem(item){
        ...
        this.news_edit = item
        this.news_edit.status_tampilan_artikel = (this.news_edit.status_tampilan_artikel === 1)
        // console.log(item)
      ...
      async submitEditItem(){
        ...
        formData.append('id_kegiatan', this.news_edit.id_kegiatan);
        ...
        axios.defaults.headers.common['Authorization'] = `Bearer ${this.token}`;
        await axios.post("/api/update-artikel", formData, config).then(response => {
          console.log(response.data)
      ...
      async submitViewItem(){
      },
      async submitCreateItem(){
        // console.log(this.news)
        let status_form =this.$refs.observer_create_news.validate()
        if(status_form){
          // start of upload
          this.overlay = true
          ...
          await axios.post("/api/insert-artikel", formData, config).then(response => {
            // console.log(response.data)
            if(response.data.status == false){
              this.teksSnackbar= "Terjadi Kesalahan : "+JSON.stringify(response.data.message),
              this.warnaSnackbar= "red"
            }else{
              this.teksSnackbar= "Berhasil menambahkan data artikel ",
          ...
          this.teksSnackbar= "Ada isian wajib yang belum diisi"
          this.warnaSnackbar= "red"
          this.snackbar = true
        }
    ...
    async mounted() {
      ...
      await this.initMap();
      console.log(this.user_memberships)
      this.overlay = false
    },
    async created(){
      axios.defaults.headers.common['Authorization'] = `Bearer ${this.token}`;
