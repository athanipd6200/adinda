L<template>
  <v-container>
    ...
    >
      ...
      <template v-slot:item.penyunting_artikel="{ item }">
      ...
      <template v-slot:item.actions="{ item }">
        ...
        <v-btn text
          small
          v-if="item.status_verifikasi_artikel === 1"
          class="mr-2"
  ...
  export default {
    ...
    data () {
      return {
        ...
        dessertHeaders: [
          ...
          { text: 'Penyunting Artikel', value: 'penyunting_artikel' },
          { text: 'Status Verifikasi', value: 'status_verifikasi_artikel' },
          { text: 'Email Verifikator', value: 'verificated_by' },
    ...
    methods: {
      ...
      async submitViewItem(){
        ...
        axios.defaults.headers.common['Authorization'] = `Bearer ${this.token}`;
        await axios.post("/api/restore-data-aset", formData, config).then(response => {
          console.log(response.data)
